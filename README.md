...
az login
```
Run in the default region (eastus) configured in the script:
```cmd
npm run test:workspace
```
List tests without executing (useful for validation):
```cmd
set PLAYWRIGHT_SERVICE_URL=https://eastus.api.playwright.microsoft.com
npx playwright test -c playwright.service.config.ts --list --reporter=line
```
Run in a different region (example: centralus):
```cmd
set PLAYWRIGHT_SERVICE_URL=https://centralus.api.playwright.microsoft.com
